...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-4 right-4 z-50 max-w-sm w-full shadow-lg rounded-lg border ${colors[type]} p-4`}
    >
...
const ExpenseDetailsPage = ({
  ...
  useEffect(() => {
    console.log("Details useEffect triggered:", {
      expenseId,
      detailsLoaded,
      loading,
    });
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-semibold">
                  Expense #{currentExpense.CodeNumber || currentExpense.Id}
                </h1>
              </div>
              <Badge
                variant={
                  currentExpense.Status === "Approved" ? "success" : "warning"
                }
              >
        ...
        <div className="bg-white border-x border-gray-200 px-6 py-4">
          <div className="flex items-center space-x-3">
            <Button icon={Edit} onClick={() => onEdit(currentExpense)}>
              Edit
            </Button>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <strong>Amount:</strong> {currentExpense.Currency}{" "}
                ...
                <div>
                  <strong>Status:</strong> {currentExpense.Status}
                </div>
                {currentExpense.Notes && (
                  <div>
                    <strong>Notes:</strong> {currentExpense.Notes}
                  </div>
                )}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  {currentExpense.ReferenceNumber || "N/A"}
                </div>
                <div>
                  <strong>Currency:</strong> {currentExpense.Currency}
                </div>
                <div>
                  <strong>Tax Amount:</strong> {currentExpense.TaxAmount || 0}
                </div>
                <div>
                  <strong>Total Amount:</strong>{" "}
                  {currentExpense.TotalAmount || currentExpense.Amount}
                </div>
                <div>
                  <strong>Created:</strong>{" "}
                  {new Date(
                    currentExpense.CreatedDate || currentExpense.CreatedAt
                  ).toLocaleDateString()}
                </div>
              </div>
            </div>
...
const FinanceExpense = () => {
  ...
  const getCategoryName = (categoryId) => {
    const category = expenseCategories?.$values?.find(
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="bg-gradient-to-r from-purple-400 to-purple-700 text-white">
            <div className="p-6">
              ...
              <p className="text-2xl font-bold mt-2">
                SAR {statistics?.TotalAmount?.toLocaleString() || "0"}
              </p>
            </div>
          </Card>
          <Card className="bg-gradient-to-r from-purple-400 to-purple-700 text-white">
            <div className="p-6">
              <h3 className="text-sm font-medium opacity-90">Average Amount</h3>
              <p className="text-2xl font-bold mt-2">
                SAR{" "}
                {Math.round(
                  statistics?.AverageExpenseAmount || 0
                ).toLocaleString()}
              </p>
            </div>
          </Card>
                ...
                {expensesList.map((expense) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge
                        variant={
                          expense.Status === "Approved" ? "success" : "warning"
                        }
                      >
                        {expense.Status}
                      </Badge>
