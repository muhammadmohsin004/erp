...
const NewIncome = () => {
  ...
  const { createIncome, updateIncome, incomeLoading } = useFinance();
  ...
  const translations = {
    ...
    "Edit Income": language === "ar" ? "تعديل دخل" : "Edit Income",
    "Clone Income": language === "ar" ? "نسخ دخل" : "Clone Income",
    ...
    "Income created successfully": language === "ar" ? "تم إنشاء الدخل بنجاح" : "Income created successfully",
    "Income updated successfully": language === "ar" ? "تم تحديث الدخل بنجاح" : "Income updated successfully",
    "Failed to create income": language === "ar" ? "فشل في إنشاء الدخل" : "Failed to create income",
    "Failed to update income": language === "ar" ? "فشل في تحديث الدخل" : "Failed to update income",
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      setFormData(prev => ({
        ...prev,
        AttachmentPath: `/uploads/income/${file.name}`
      }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && editData) {
        ...
        result = await updateIncome(editData.Id, apiData);
        if (result) {
          alert(translations["Income updated successfully"]);
          navigate("/admin/finance/incomes");
        } else {
          alert(translations["Failed to update income"]);
        }
      } else {
        // Create new income
        result = await createIncome(apiData);
        if (result) {
          alert(translations["Income created successfully"]);
          navigate("/admin/finance/incomes");
        } else {
          alert(translations["Failed to create income"]);
        }
      }
    } catch (error) {
      console.error("Error submitting income:", error);
      alert(isEditing ? translations["Failed to update income"] : translations["Failed to create income"]);
    } finally {
  ...
  const getPageTitle = () => {
    if (isEditing) return translations["Edit Income"];
    if (isCloning) return translations["Clone Income"];
    return translations["Add Income"];
  };
        ...
        <Container className="flex items-center justify-between mb-6">
          <Container className="flex items-center gap-4">
            <FilledButton
              ...
              height="h-10"
              width="w-10"
              onClick={() => navigate("/admin/finance/incomes")}
            />
          ...
          <FilledButton
            ...
            disabled={isSubmitting || incomeLoading}
          />
        </Container>
          ...
          <Container className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <Container className="space-y-4">
              ...
              {(uploadedFile || formData.AttachmentPath) && (
                <Container className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
