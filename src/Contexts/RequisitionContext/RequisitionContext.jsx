...
const requisitionReducer = (state, action) => {
  switch (action.type) {
    ...
    case actionTypes.ADD_REQUISITION:
      // Handle adding requisition to the nested structure
      { const currentRequisitionsAdd = state.requisitions?.Data || [];
      const updatedRequisitionsAdd = [...currentRequisitionsAdd, action.payload];
      return { 
        ...state, 
        requisitions: {
          ...state.requisitions,
    ...
    case actionTypes.UPDATE_REQUISITION:
      // Handle updating requisition in the nested structure
      { const currentRequisitionsUpdate = state.requisitions?.Data || [];
      const updatedRequisitionsUpdate = currentRequisitionsUpdate.map(requisition =>
      ...
      return {
        ...
        currentRequisition: state.currentRequisition?.Id === action.payload.Id 
          ? action.payload 
    ...
    case actionTypes.DELETE_REQUISITION:
      // Handle deleting requisition from the nested structure
      { const currentRequisitionsDelete = state.requisitions?.Data || [];
      ...
      return {
        ...
        currentRequisition: state.currentRequisition?.Id === action.payload 
          ? null 
          : state.currentRequisition,
        loading: false,
        error: null
    ...
    case actionTypes.ADD_ITEM:
      return {
        ...
        currentRequisition: state.currentRequisition ? {
          ...state.currentRequisition,
          Items: [...(state.currentRequisition.Items || []), action.payload]
        } : state.currentRequisition,
        loading: false,
        error: null
      };
    ...
    case actionTypes.ADD_ATTACHMENTS:
      return {
        ...
        currentRequisition: state.currentRequisition ? {
          ...state.currentRequisition,
          Attachments: [...(state.currentRequisition.Attachments || []), ...action.payload]
        } : state.currentRequisition,
        loading: false,
        error: null
      };
...
export const RequisitionProvider = ({ children }) => {
  ...
  const getRequisitions = useCallback(async (params = {}) => {
    try {
      ...
      if (params.sortAscending !== undefined || state.filters.sortAscending !== undefined) {
        queryParams.append('sortAscending', params.sortAscending ?? state.filters.sortAscending);
      }
      ...
      if (response.Success) {
        ...
        if (response.Pagination) {
          ...
          dispatch({ type: actionTypes.SET_PAGINATION, payload: {
            ...
            HasNextPage: currentPage < Math.ceil(requisitionsData.length / pageSize)
          }});
        }
      } else {
  ...
  const getRequisition = useCallback(async (id) => {
    try {
      ...
      if (response.Success) {
        dispatch({ type: actionTypes.SET_CURRENT_REQUISITION, payload: response.Data });
  ...
  const updateRequisition = useCallback(async (id, requisitionData) => {
    try {
      ...
      if (response.Success) {
        dispatch({ type: actionTypes.UPDATE_REQUISITION, payload: response.Data });
        return response.Data;
      } else {
  ...
  const addItemToRequisition = useCallback(async (requisitionId, itemData) => {
    try {
      ...
      const response = await makeApiCall(`${API_BASE_URL}/${requisitionId}/items`, {
        method: 'POST',
  ...
  const uploadAttachments = useCallback(async (requisitionId, files) => {
    try {
      ...
      const response = await makeFileUploadCall(`${API_BASE_URL}/${requisitionId}/attachments`, formData);
      
      if (response.Success && response.Data?.UploadedAttachments) {
        dispatch({ type: actionTypes.ADD_ATTACHMENTS, payload: response.Data.UploadedAttachments });
        return response.Data;
      } else {
  ...
  // Download attachment
  const downloadAttachment = useCallback(async (requisitionId, fileName, originalFileName) => {
    try {
      const token = getAuthToken();
      const response = await fetch(`${API_BASE_URL}/${requisitionId}/attachments/${fileName}`, {
        headers: {
          ...(token && { 'Authorization': `Bearer ${token}` })
        }
      });
  ...
  const deleteAttachment = useCallback(async (requisitionId, attachmentId) => {
    try {
      ...
      const response = await makeApiCall(`${API_BASE_URL}/${requisitionId}/attachments/${attachmentId}`, {
        method: 'DELETE'
      });
      
      if (response.Success) {
        dispatch({ type: actionTypes.DELETE_ATTACHMENT, payload: attachmentId });
        return true;
      } else {
  ...
  const searchRequisitions = useCallback(async (searchTerm) => {
  ...
  const filterRequisitionsByType = useCallback(async (type) => {
  ...
  const sortRequisitions = useCallback(async (sortBy, sortAscending = false) => {
