  ...
  switch (action.type) {
    ...
    case actionTypes.UPDATE_INCOME_CATEGORY:
      ...
      return {
        ...
        currentIncomeCategory: state.currentIncomeCategory?.Id === action.payload.Id 
          ? action.payload 
    ...
    case actionTypes.DELETE_INCOME_CATEGORY:
      ...
      return {
        ...
        currentIncomeCategory: state.currentIncomeCategory?.Id === action.payload 
          ? null 
          : state.currentIncomeCategory,
        loading: false,
        error: null
    ...
    case actionTypes.TOGGLE_INCOME_CATEGORY_STATUS:
      ...
      return {
        ...
        currentIncomeCategory: state.currentIncomeCategory?.Id === action.payload.id 
          ? { ...state.currentIncomeCategory, IsActive: action.payload.isActive }
          : state.currentIncomeCategory,
        loading: false,
        error: null
      };
...
export const IncomeCategoryProvider = ({ children }) => {
  ...
  const getIncomeCategories = useCallback(async (params = {}) => {
    try {
      ...
      if (response.Success) {
        ...
        throw new Error(response.Message || 'Failed to fetch income categories');
      }
    } catch (error) {
  ...
  const getIncomeCategory = useCallback(async (id) => {
    try {
      ...
      if (response.Success) {
        dispatch({ type: actionTypes.SET_CURRENT_INCOME_CATEGORY, payload: response.Data.Category });
        if (response.Data.RecentIncomes) {
          dispatch({ type: actionTypes.SET_RECENT_INCOMES, payload: response.Data.RecentIncomes });
        }
  ...
  const createIncomeCategory = useCallback(async (categoryData) => {
    try {
      ...
      if (response.Success) {
        dispatch({ type: actionTypes.ADD_INCOME_CATEGORY, payload: response.Data });
        return response.Data;
      } else {
  ...
  const updateIncomeCategory = useCallback(async (id, categoryData) => {
    try {
      ...
      if (response.Success) {
        dispatch({ type: actionTypes.UPDATE_INCOME_CATEGORY, payload: response.Data });
        return response.Data;
      } else {
  ...
  const toggleIncomeCategoryStatus = useCallback(async (id) => {
    try {
      dispatch({ type: actionTypes.SET_LOADING, payload: true });
      ...
      if (response.Success) {
        ...
        const currentCategory = state.incomeCategories?.Data?.find(cat => cat.Id === id);
        ...
        throw new Error(response.Message || 'Failed to toggle income category status');
      }
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
      return false;
  ...
  const getCategoryIncomes = useCallback(async (id, params = {}) => {
    try {
      ...
      const response = await makeApiCall(`${API_BASE_URL}/${id}/incomes?${queryParams}`);
      
      if (response.Success) {
        dispatch({ type: actionTypes.SET_CATEGORY_INCOMES, payload: response.Data });
        return response.Data;
      } else {
  ...
  const getCategoryPerformance = useCallback(async (startDate = null, endDate = null) => {
    try {
      dispatch({ type: actionTypes.SET_LOADING, payload: true });
      ...
      const response = await makeApiCall(`${API_BASE_URL}/performance?${queryParams}`);
      
      if (response.Success) {
        dispatch({ type: actionTypes.SET_CATEGORY_PERFORMANCE, payload: response.Data });
        return response.Data;
      } else {
  ...
  const searchIncomeCategories = useCallback(async (searchTerm) => {
  ...
  const filterIncomeCategoriesByStatus = useCallback(async (isActive) => {
  ...
  const sortIncomeCategories = useCallback(async (sortBy, sortAscending = true) => {
  ...
  const getActiveIncomeCategories = useCallback(() => {
    return state.incomeCategories?.Data?.filter(category => category.IsActive) || [];
  }, [state.incomeCategories]);
  ...
  const getCategoryById = useCallback((id) => {
    return state.incomeCategories?.Data?.find(category => category.Id === id) || null;
  }, [state.incomeCategories]);
