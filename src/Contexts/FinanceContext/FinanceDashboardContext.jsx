...
const apiCall = async (url, options = {}) => {
  ...
  try {
    ...
    if (contentType && contentType.includes("text/html")) {
      throw new Error(
        `API endpoint returned HTML instead of JSON. Check if the endpoint ${url} exists.`
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON response from server. The API might be returning HTML or malformed data.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Network error. Check if the API server is running and accessible."
      );
    }
...
const transformCashFlowData = (data) => {
  ...
  const chartData =
    data.DailyBreakdown.$values?.map((day) => ({
      period: new Date(day.Date).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      }),
...
const transformExpenseAnalysis = (data) => {
  ...
  const topCategories =
    data.CategoryBreakdown.$values?.map((category) => ({
...
const transformPendingApprovals = (data) => {
  ...
  const items =
    data.PendingExpenses.$values?.map((expense) => ({
...
export const FinanceDashboardProvider = ({ children }) => {
  ...
  // Dashboard Overview
  const getFinanceOverview = useCallback(
  ...
  const getCurrentBalance = useCallback(async () => {
    try {
      ...
      const transformedData = {
        totalBalance:
          response.Data?.TotalBankBalance || response.Data?.CurrentBalance || 0,
  ...
  const getCashFlowAnalysis = useCallback(
    async (startDate = null, endDate = null, period = "daily") => {
      try {
        const params = { period };
        if (startDate) params.startDate = startDate;
        if (endDate) params.endDate = endDate;
  ...
  const getProfitLossSummary = useCallback(
    async (startDate = null, endDate = null) => {
      try {
        ...
        const queryString = buildQueryString(params);
        const url = `${API_BASE_URL}/profit-loss${
          queryString ? `?${queryString}` : ""
        }`;
  ...
  // Expense Analysis
  const getExpenseAnalysis = useCallback(
    async (startDate = null, endDate = null, topCategories = 10) => {
      try {
        ...
        dispatch({
          type: ActionTypes.SET_EXPENSE_ANALYSIS,
          payload: transformedData,
        });
        return transformedData;
      } catch (error) {
  ...
  // Income Analysis
  const getIncomeAnalysis = useCallback(
    async (startDate = null, endDate = null, topCategories = 10) => {
      try {
        const params = { topCategories };
        if (startDate) params.startDate = startDate;
        ...
        dispatch({
          type: ActionTypes.SET_INCOME_ANALYSIS,
          payload: transformedData,
        });
        return transformedData;
      } catch (error) {
  ...
  const getFinancialTrends = useCallback(
    async (period = "monthly", periods = 12) => {
      try {
  ...
  const getPendingApprovals = useCallback(async () => {
    try {
      ...
      dispatch({
        type: ActionTypes.SET_PENDING_APPROVALS,
        payload: transformedData,
      });
      return transformedData;
    } catch (error) {
  ...
  const refreshDashboard = useCallback(
    async (customFilters = {}) => {
      ...
      try {
        ...
        const promises = [
          getFinanceOverview(filters.startDate, filters.endDate).catch(
            (err) => ({ error: err.message, type: "overview" })
          ),
          ...
          getCashFlowAnalysis(
            ...
            filters.endDate,
            filters.period
          ).catch((err) => ({ error: err.message, type: "cashFlow" })),
          getProfitLossSummary(filters.startDate, filters.endDate).catch(
            (err) => ({ error: err.message, type: "profitLoss" })
          ),
          getExpenseAnalysis(
            filters.startDate,
            filters.endDate,
            filters.topCategories
          ).catch((err) => ({ error: err.message, type: "expenseAnalysis" })),
          getIncomeAnalysis(
            filters.startDate,
            filters.endDate,
            filters.topCategories
          ).catch((err) => ({ error: err.message, type: "incomeAnalysis" })),
          getFinancialTrends(filters.period, filters.periods).catch((err) => ({
            error: err.message,
            type: "trends",
          })),
...
export const useFinanceDashboard = () => {
  ...
  if (!context) {
    throw new Error(
      "useFinanceDashboard must be used within a FinanceDashboardProvider"
    );
  }
  return context;
};
