...
export const StockProvider = ({ children }) => {
  ...
  const getProductStockTransactions = useCallback(
    async (productId, params = {}) => {
      try {
        ...
        queryParams.append(
          "page",
          params.page || state.pagination.PageNumber || 1
        );
        queryParams.append(
          "pageSize",
          params.pageSize || state.pagination.PageSize || 25
        );
        ...
        if (params.dateFrom || state.filters.dateFrom) {
          queryParams.append(
            "dateFrom",
            formatDateForApi(params.dateFrom || state.filters.dateFrom)
          );
        }
        if (params.dateTo || state.filters.dateTo) {
          queryParams.append(
            "dateTo",
            formatDateForApi(params.dateTo || state.filters.dateTo)
          );
        }
        ...
        const response = await makeApiCall(
          `${API_BASE_URL}/product/${productId}/transactions?${queryParams}`
        );
        ...
        if (response.Success) {
          ...
          // Handle pagination from response - check for both possible property names
          const paginationData = response.Pagination || response.Paginations;
          if (paginationData) {
            dispatch({
              type: actionTypes.SET_PAGINATION,
              payload: {
                CurrentPage:
                  paginationData.CurrentPage || paginationData.PageNumber,
          ...
          throw new Error(
            response.Message || "Failed to fetch stock transactions"
          );
        }
      } catch (error) {
  ...
  const getStockMovements = useCallback(
    async (params = {}) => {
      try {
        ...
        const response = await makeApiCall(
          `${API_BASE_URL}/movements?${queryParams}`
        );
        if (response.Success) {
          dispatch({
            type: actionTypes.SET_STOCK_MOVEMENTS,
            payload: response,
          });

          // Handle pagination from response - use correct property name
          ...
          throw new Error(
            response.Message || "Failed to fetch stock movements"
          );
        }
      } catch (error) {
        dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const getStockSummary = useCallback(async () => {
    try {
      ...
      if (response.Success) {
        dispatch({
          type: actionTypes.SET_STOCK_SUMMARY,
  ...
  const addStockTransaction = useCallback(
    async (productId, transactionData) => {
      try {
        dispatch({ type: actionTypes.SET_LOADING, payload: true });
        ...
        if (response.Success) {
          ...
          throw new Error(
            response.Message || "Failed to add stock transaction"
          );
        }
      } catch (error) {
        dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const adjustStock = useCallback(
    async (productId, adjustmentData) => {
      try {
        dispatch({ type: actionTypes.SET_LOADING, payload: true });

        const response = await makeApiCall(
          `${API_BASE_URL}/product/${productId}/adjust`,
          {
            method: "POST",
  ...
  const transferStock = useCallback(
    async (productId, transferData) => {
      try {
        dispatch({ type: actionTypes.SET_LOADING, payload: true });

        const response = await makeApiCall(
          `${API_BASE_URL}/product/${productId}/transfer`,
          {
            method: "POST",
  ...
  const filterStockMovements = useCallback(
    async (filters) => {
  ...
  const changePageSize = useCallback(
    async (pageSize) => {
      ...
      if (state.filters.productId) {
        await getProductStockTransactions(state.filters.productId, {
          pageSize,
          page: 1,
        });
      } else {
  ...
  const searchMovements = useCallback(
    async (searchTerm) => {
      ...
      if (!searchTerm.trim()) {
        ...
        const allMovements = state.stockMovements?.Data?.$values || [];
        const filteredMovements = allMovements.filter(
          (movement) =>
            movement.ProductName?.toLowerCase().includes(
              searchTerm.toLowerCase()
            ) ||
            movement.WarehouseName?.toLowerCase().includes(
              searchTerm.toLowerCase()
            ) ||
            movement.MovementType?.toLowerCase().includes(
              searchTerm.toLowerCase()
            ) ||
            movement.Reference?.toLowerCase().includes(
              searchTerm.toLowerCase()
            ) ||
        ...
        dispatch({
          ...
          payload: {
            ...
            Data: {
              ...state.stockMovements?.Data,
              $values: filteredMovements,
            },
          },
        });
      }
