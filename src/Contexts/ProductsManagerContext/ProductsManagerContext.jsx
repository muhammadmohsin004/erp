  ...
  switch (action.type) {
    ...
    case actionTypes.UPDATE_PRODUCT_BRAND: {
      ...
      return {
        ...
        currentProductBrand:
          state.currentProductBrand?.Id === action.payload.Id
            ? action.payload
    ...
    case actionTypes.DELETE_PRODUCT_BRAND: {
      ...
      return {
        ...
        currentProductBrand:
          state.currentProductBrand?.Id === action.payload
            ? null
            : state.currentProductBrand,
        loading: false,
        error: null,
    ...
    case actionTypes.UPDATE_PRODUCT_CATEGORY: {
      const currentCategoriesUpdate =
        state.productCategories?.Data?.$values || [];
      ...
      return {
        ...
        currentProductCategory:
          state.currentProductCategory?.Id === action.payload.Id
            ? action.payload
    ...
    case actionTypes.DELETE_PRODUCT_CATEGORY: {
      ...
      return {
        ...
        currentProductCategory:
          state.currentProductCategory?.Id === action.payload
            ? null
            : state.currentProductCategory,
        loading: false,
        error: null,
    ...
    case actionTypes.ADD_MULTIPLE_PRODUCT_IMAGES: {
      ...
      const updatedImagesAddMultiple = [
        ...currentImagesAddMultiple,
        ...action.payload,
      ];
      return {
        ...state,
...
export const ProductsManagerProvider = ({ children }) => {
    ...
    async (params = {}) => {
      try {
        ...
        if (params.lowStockOnly || state.filters.lowStockOnly) {
          queryParams.append(
            "lowStockOnly",
            params.lowStockOnly || state.filters.lowStockOnly
          );
        }
  ...
  const getProductBrands = useCallback(async (params = {}) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.productBrands}?${queryParams}`
      );
  ...
  const getProductBrand = useCallback(async (id) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.productBrands}/${id}`
      );

      if (response.Success) {
        dispatch({
          type: actionTypes.SET_CURRENT_PRODUCT_BRAND,
          payload: response.Data,
        });
        return response.Data;
      } else {
  ...
  const updateProductBrand = useCallback(async (id, brandData) => {
    try {
      ...
      if (response.Success) {
        dispatch({
          type: actionTypes.UPDATE_PRODUCT_BRAND,
          payload: response.Data,
        });
        return response.Data;
      } else {
  ...
  const getProductCategories = useCallback(async (params = {}) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.productCategories}?${queryParams}`
      );
      ...
      if (response.Success) {
        ...
        throw new Error(
          response.Message || "Failed to fetch product categories"
        );
      }
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const getProductCategory = useCallback(async (id) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.productCategories}/${id}`
      );

      if (response.Success) {
        dispatch({
          type: actionTypes.SET_CURRENT_PRODUCT_CATEGORY,
          payload: response.Data,
        });
        return response.Data;
      } else {
  ...
  const getCategoriesTree = useCallback(async () => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.productCategories}/tree`
      );
  ...
  const getProductImagesByProductId = useCallback(async (productId) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.productImages}/ByProduct/${productId}`
      );
  ...
  const createMultipleProductImages = useCallback(async (formData) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Multiple upload failed: ${response.status} - ${errorText}`
        );
      }
      ...
      if (result.Success) {
        ...
        throw new Error(
          result.Message || "Failed to create multiple product images"
        );
      }
    } catch (error) {
  ...
  const getStatisticsOverview = useCallback(async () => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.statistics}/overview`
      );

      dispatch({
        type: actionTypes.SET_STATISTICS,
        payload: { overview: response },
      });
      return response;
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const getStatisticsCategories = useCallback(async () => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.statistics}/categories`
      );
  ...
  const getStatisticsTopProducts = useCallback(
    async (metric = "price", limit = 10) => {
      try {
        dispatch({ type: actionTypes.SET_LOADING, payload: true });

        const response = await makeApiCall(
          `${API_BASE_URLS.statistics}/top-products?metric=${metric}&limit=${limit}`
        );

        dispatch({
          type: actionTypes.SET_STATISTICS,
          payload: { topProducts: response },
        });
        return response;
      } catch (error) {
        dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const getStatisticsTrends = useCallback(async (months = 6) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.statistics}/trends?months=${months}`
      );

      dispatch({
        type: actionTypes.SET_STATISTICS,
        payload: { trends: response },
      });
      return response;
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
    ...
    try {
      ...
      if (response.Success) {
        dispatch({
          ...
          payload: { products: response.Data.$values },
        });
        return response.Data;
      } else {
        throw new Error(
          response.Message || "Failed to fetch products dropdown"
        );
      }
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const getCategoriesDropdown = useCallback(async () => {
    try {
      const response = await makeApiCall(
        `${API_BASE_URLS.productCategories}/dropdown`
      );

      if (response.Success) {
        dispatch({
          type: actionTypes.SET_DROPDOWNS,
          payload: { categories: response.Data.$values },
        });
        return response.Data;
      } else {
        throw new Error(
          response.Message || "Failed to fetch categories dropdown"
        );
      }
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
  ...
  const getBrandsDropdown = useCallback(async () => {
    try {
      const response = await makeApiCall(
        `${API_BASE_URLS.productBrands}/dropdown`
      );

      if (response.Success) {
        dispatch({
          type: actionTypes.SET_DROPDOWNS,
          payload: { brands: response.Data.$values },
        });
        return response.Data;
      } else {
  ...
  const searchProducts = useCallback(async (query = "", limit = 10) => {
    try {
      const response = await makeApiCall(
        `${API_BASE_URLS.products}/search?query=${encodeURIComponent(
          query
        )}&limit=${limit}`
      );
  ...
  const getProductByBarcode = useCallback(async (barcode) => {
    try {
      ...
      const response = await makeApiCall(
        `${API_BASE_URLS.products}/barcode/${encodeURIComponent(barcode)}`
      );
      ...
      if (response.Success) {
        ...
        throw new Error(
          response.Message || "Failed to fetch product by barcode"
        );
      }
    } catch (error) {
      dispatch({ type: actionTypes.SET_ERROR, payload: error.message });
