...
updateClient: async (clientId, clientData, attachments = []) => {
  try {
    ...
    Object.keys(sanitizedData).forEach(key => {
      if (sanitizedData[key] !== null && sanitizedData[key] !== undefined && sanitizedData[key] !== '') {
        if (key === 'contacts' && Array.isArray(sanitizedData[key])) {
          ...
          // Skip this as we'll handle it separately below
          return;
        } else {
          // Use PascalCase for backend compatibility
          const backendKey = key.charAt(0).toUpperCase() + key.slice(1);
    ...
    // Handle AttachmentsToRemove field (required by backend)
    ...
    if (Array.isArray(attachmentsToRemove) && attachmentsToRemove.length > 0) {
      // If there are specific attachments to remove
      attachmentsToRemove.forEach((attachmentId, index) => {
        formData.append(`AttachmentsToRemove[${index}]`, attachmentId);
      });
    } else {
      // Even if no attachments to remove, we need to include this field
      // Some backends expect at least an empty value
      formData.append('AttachmentsToRemove', '');
    }
